...
import { GetAsbTipeBangunanByAsbJenisDto } from "./dto/get_asb_tipe_bangunan_by_asb_jenis.dto";
...
export class AsbTipeBangunanController {
  ...
  @Get("by-asb-jenis")
  @Roles(Role.SUPERADMIN)
  async getAsbTipeBangunanByAsbJenis(@Body() dto: GetAsbTipeBangunanByAsbJenisDto): Promise<ResponseDto> {
    try {
      const result = await this.asbTipeBangunanService.getByAsbJenis(dto);

      return {
        status: "success",
        responseCode: HttpStatus.OK,
        message: "AsbTipeBangunan by AsbJenis retrieved",
        data: result,
      };
    } catch (error) {
      if (error instanceof HttpException) {
